...
static uint
bmap(struct inode *ip, uint bn,uint checksum,int rwFlag)
{
...
{
	//off-> current off of the file . off/BSIZE -> current block of the file
  uint tot, m;
  struct buf *bp;
...
dirlookup(struct inode *dp, char *name, uint *poff)
{
  ...
  for(off = 0; off < dp->size; off += BSIZE){
    bp = bread(dp->dev, bmap(dp, off / BSIZE,0,-1));
